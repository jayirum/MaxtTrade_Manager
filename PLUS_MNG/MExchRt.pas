...
begin
  ...
  with DataSet do begin
    TFloatField(FieldByName('ER_RT')).DisplayFormat := FORMAT_EXCHANGE;
  end;
end;
...
begin
  ...
  with DataSet do begin
    if FieldByName('ER_RT').AsString = '' then begin
      bsMsgError('환율은 반드시 입력하셔야 합니다');
      Abort;
    end;
    ...
    if State in [dsInsert] then begin
      ...
      FieldByName('MNG_ID').AsString := _Login_ID;

      dErRt := FieldByName('ER_RT').AsFloat;

      sSql := Format(
        'UPDATE ARTC_MST ' +
        '   SET TICK_VALUE     = ROUND(TICK_VALUE_USD * %g, 0,1) ' +
...
procedure TfmExchRt.FormCreate(Sender: TObject);
begin
  inherited;
  SetADOConn(fmExchRt);
end;
