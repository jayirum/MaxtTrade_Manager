...
uses StdUtils, PacketStruct, PacketThread, MBasic, MUser, MMastDB, MLogin,
  MPart, MNego, MInOut, MCorp, MStk, MNoti, MOrdDetail, MNclr, MNoCntrOrd, MArtc,
  ...
  MPresent, MPower, MRealPL, GTUpdate, MUserArtc, MOvApprove, MOvStatus, MUserOv,
  MStkPL, MUserStkPL, MConnIP,
  MExchRt{, uChatClient};
 // MScL, MAccount,
...
begin
  if InputBox('GT DB TOOLS', 'Password', '') = 'gtrade09' then begin // TODO : CFG 파일에 추가할것
...
//  sPc001: String;
  ...
  sList: TStringList;
begin
//  G_MANAGER_TF := True; // True : Manager가 접속함을 뜻함
  ...
  _UPDATE_DNS1 := Get_CFGFile('UPDATE_SERVER', 'ADDRESS1', 'gtrade.codns.com', True, _CFGServerIP);
  _UPDATE_DNS2 := Get_CFGFile('UPDATE_SERVER', 'ADDRESS2', 'gtrade.codns.com', True, _CFGServerIP);
...
// TODO : 메인메뉴와 윈도우스킨선텍 버특의 스킨 리스트 다름, 하나로 합칠것, FormClose에 스킨저장에 반영할것
  bsSkinData.SkinIndex := StrToIntDef(Get_CFGFile('GT_SKIN', 'INDEX', '6'), 6);

  // App Update ----------------------------------------------------------------
// TODO : 프로그램 테스트 완료 후 Comment 처리하지 말것
{  if Not bUpdateRun then begin
    if AutoUpdate_Run(_UPDATE_DNS1, 'mng_info.inf', True, False) then Exit       // 1차 URL
    else if AutoUpdate_Run(_UPDATE_DNS2, 'mng_info.inf') then Exit;              // 2차 URL
  end else begin
...
procedure TfmMain.btnChatClick(Sender: TObject);
begin
//  Form_ChatClient;
end;

{procedure TfmMain.Form_ChatClient;
var
  i: Integer;
  bTF: Boolean;
...
begin
  ...
  for i:=0 to Application.MainForm.MDIChildCount-1 do begin
    if Application.MainForm.MDIChildren[i].ClassName = TfrmChatClient.ClassName then begin
      bTF := True;
  ...
  //Application.CreateForm(TfrmChatServer, frmChatServer);
  frmChatClient := TfrmChatClient.Create(self);
  // MDI
  frmChatClient.FormStyle   := fsMDIChild;
  frmChatClient.WindowState := wsNormal;
